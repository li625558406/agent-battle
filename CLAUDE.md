...
- [CHANGE.md](./CHANGE.md) — 迭代记录。当前进度：**M1 与 M2 三件交付物（六维画像、复盘报告、--dry-run 影子赛）均已完成**——平台注册/任务下发/结果上报/Elo 结算/天梯全链路联通，并发与孤儿对局已加固；结算后自动重建六维能力画像（全体池百分位归一化），`profile` 子命令与 API 可查；任一已结算对局可按 matchID 查得结构完整复盘；`mirror --dry-run` 零出网影子赛可逐字节预览平台将收到的全部数据。M3 公测项（匹配系统、盲评、Web Dashboard）待启动。平台/Runner 代码分别在 `platform/`、`runner/` 子树。
