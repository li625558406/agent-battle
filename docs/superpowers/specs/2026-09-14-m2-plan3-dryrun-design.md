...
日期：2026-09-14
状态：已获用户批准；终审后口径修订（2026-09-14，随实现定稿）：① §2"`--task-id` 仍必填"实际实现为缺省取 `--task` 目录名（与 `--server` 模式既有口径一致）；② §6 dump IO 失败"立即中止"实际实现为流程收尾查 `DumpError` 非零退出（失败请求在 stdout 摘要逐条标注，不在 Report 回调内中断以保持与真实上报同轨）。
